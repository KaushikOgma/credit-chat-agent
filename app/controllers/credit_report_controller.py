...
class CreditReportController:
    ...
    async def get_credit_report_context(self, db: Database, user_id: str, user_query: str) -> Union[None, str]:
        ...
        try:
            ...
            report = await self.credit_report_repo.get_todays_reoprt(db, user_id)
            print("[DEBUG] Fetched report:", report)

            if report:
                if not report["isVectorized"]:
                    mongo_data, vector_data = await self.credit_report_processor_service.process_report(
                        credit_report_json=None,
                        user_id=user_id,
                        categorized_resp=report["report"]
                    )
                    await self.vectorizer.create_vectorstore(vector_data, "report_data_id", "topics")
                    await self.credit_report_repo.update_report(db, report["_id"], {"isVectorized": True})
            else:
                ...
                if credit_report:
                    mongo_data, vector_data = await self.credit_report_processor_service.process_report(
                        user_id=user_id,
                        credit_report_json=credit_report,
                        categorized_resp=None
                    )
                    if mongo_data:
                        mongo_data["isVectorized"] = False
                        inserted_id = await self.credit_report_repo.add_report(db, mongo_data)
                        if not self.vectorizer.vectordb:
                            self.vectorizer.load_vectorstore()
                        await self.vectorizer.create_vectorstore(vector_data, "report_data_id", "topics")
                        updates = {"isVectorized": True, "other_field": "value"}
                        await self.credit_report_repo.update_report(db, inserted_id, updates)

            # Fetch relevant context for user_query
            user_context = await self.vectorizer.query_vectorstore(user_query)

            # Return the context as expected in the graph flow
            return {"credit_report_context": user_context}

        except Exception as e:
            print(f"[ERROR] get_credit_report_context failed: {e}")
            return None
