...
tools = [
    initialization_node,
    load_history_node,
    fetch_today_report_node,
    fetch_and_sync_new_data_node,
    fetch_vector_db_node,
    populate_vector_db_node,
    pull_model_config_node,
    conversational_agent_node,
    persist_messages_node,
    deinitialization_node,
    error_handler_node
]

def eval(state: Dict[str, Any]) -> Dict[str, Any]:
    """Evaluate the state of the workflow and return the next node to execute.
    This function checks for errors and determines the next node based on the current state.

    Args:
        state (Dict[str, Any]): The current state of the workflow.

    Returns:
        Dict[str, Any]: The next node to execute in the workflow.
    """
    return state
    

prompt = """
You are a chat agent system designed to follow a structured workflow for processing user queries.

Begin by initializing the system for the given user using their user_id: U123456.

1. Load the user's previous conversation history.
2. Fetch the user's report for today.
3. Synchronize any new external data related to the user.
4. Attempt to fetch the vector database. If it doesn't exist, initialize and populate it with relevant data.
5. Pull the current model configuration.
6. Use the model to generate a response for the given user query: "What’s the update on my credit score?".
7. Take into account whether the user is verified: True.
8. Save the entire conversation (user query + system response) for future reference.
9. Deinitialize the session for user_id: U123456 once the task is complete.

If any step encounters an error, route to the error handler and then safely deinitialize the system.

Your final goal is to return a helpful and context-aware response to the user query.
"""
