...
from app.services.qa_generator.token_counter import count_tokens_text, count_tokens_messages, get_available_tokens, are_tokens_available_for_both_conversations
from app.services.qa_generator.prompt_helper import create_question_extraction_conversation_messages, create_answer_generation_conversation_messages, create_text_cleaning_conversation_messages
...
async def clean_text(text):
    ...
    return await run_model(messages)
...
async def extract_questions_from_text(text):
    ...
    if not are_tokens_available_for_both_conversations(num_tokens_text):
        ...
        tasks = []
        for sub_text in chunk_text(text, chunk_size=1500, chunk_overlap=100):
            tasks.append(extract_questions_from_text(sub_text))
...
async def process_text():
    text = """
        ...
        the credit world.
                                   - 4 -
    """
    result = await generate_question_and_answer(text)
